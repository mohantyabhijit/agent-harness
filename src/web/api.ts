...
export interface CampaignSnapshot extends Campaign {
  ...
  readonly qodoFindings: readonly { readonly id: string; readonly severity: "high" | "medium" | "low" | "suggestion"; readonly status: "open" | "fixed" | "dismissed"; readonly summary: string; readonly sourceUrl?: string; readonly body?: string; readonly path?: string; readonly line?: number; readonly disposition?: string }[];
...
const qodoFinding = z.object({ id: z.string().trim().min(1).max(128), severity: z.enum(["high", "medium", "low", "suggestion"]), status: z.enum(["open", "fixed", "dismissed"]), summary: text, sourceUrl: z.url().max(2_048).refine(githubReviewUrl, "GitHub review URL").optional(), body: longText.optional(), path: z.string().min(1).max(1_024).refine(safeQodoPath).optional(), line: finite.int().positive().max(Number.MAX_SAFE_INTEGER).optional(), disposition: text.optional() }).strict().superRefine((finding, context) => { const sourceCount = [finding.body, finding.path, finding.line].filter((value) => value !== undefined).length; if (sourceCount !== 0 && sourceCount !== 3) context.addIssue({ code: "custom", message: "Incomplete Qodo source evidence" }); });
...
function safeQodoPath(value: string): boolean { return value === value.trim() && !value.startsWith("/") && !value.includes("\\") && value.split("/").every((segment) => segment.length > 0 && segment !== "." && segment !== ".." && !containsControlCharacter(segment)); }
function containsControlCharacter(value: string): boolean { for (let index = 0; index < value.length; index += 1) { const codeUnit = value.charCodeAt(index); if (codeUnit <= 31 || codeUnit === 127) return true; } return false; }
