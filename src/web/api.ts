...
export interface OpenQuestApiOptions { readonly fetch: FetchLike; readonly baseUrl?: string; }
...
export function createOpenQuestApi(options: OpenQuestApiOptions): OpenQuestApi {
  ...
  return {
    ...
    async discoverRepositories(selected, signal) { const body = await request(options.fetch, `${baseUrl}/api/discovery/repositories`, withSignal({ method: "POST", headers: jsonHeaders(), body: JSON.stringify({ spaces: [...new Set(selected)] }) }, signal)); return parsed(z.object({ repositories: z.array(discovered).max(100) }).strict(), body, "Recommendations could not be loaded").repositories as readonly DiscoveredRepository[]; },
    ...
    async createCampaign(input, signal) { const body = await request(options.fetch, `${baseUrl}/api/campaigns`, withSignal({ method: "POST", headers: jsonHeaders(), body: JSON.stringify(input) }, signal)); const response = parsed(campaignResponse, body, "Campaign could not be started"); return { id: response.id }; },
