...
export interface OpenQuestApi {
  ...
  discoverRepositories(spaces: readonly Space[], signal?: AbortSignal): Promise<DiscoveryResponse<DiscoveredRepository>>;
  getIssues(repository: string, signal?: AbortSignal): Promise<DiscoveryResponse<IssueCandidate>>;
...
export function createOpenQuestApi(options: OpenQuestApiOptions): OpenQuestApi {
  ...
  return {
    ...
    async discoverRepositories(selected, signal) { const validSpaces = parsed(z.array(z.enum(spaces)).length(1), selected, "Recommendations could not be loaded"); const body = await request(options.fetch, `${baseUrl}/api/discovery/repositories`, withSignal({ method: "POST", headers: authenticatedHeaders(options.operatorCapability), body: JSON.stringify({ spaces: validSpaces }) }, signal)); const response = parsed(repositoryEnvelope, body, "Recommendations could not be loaded"); return { values: response.repositories as readonly DiscoveredRepository[], ...metadataFrom(response) }; },
    async getIssues(name, signal) { const [owner, repo, extra] = name.split("/"); if (owner === undefined || repo === undefined || extra !== undefined || !repositoryName.safeParse(name).success) throw new OpenQuestApiError("Issues could not be loaded"); const body = await request(options.fetch, `${baseUrl}/api/discovery/repositories/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/issues`, withSignal({}, signal)); const response = parsed(issueEnvelope, body, "Issues could not be loaded"); if (response.issues.some((value) => value.repository !== name)) throw new OpenQuestApiError("Issues could not be loaded"); return { values: response.issues, ...metadataFrom(response) }; },
...
const envelopeMetadata = { verifiedAt: timestamp.optional(), source: z.enum(["snapshot", "live"]).optional(), refreshing: z.boolean().optional() } as const;
const repositoryEnvelope = z.object({ repositories: z.array(discovered).max(8), ...envelopeMetadata }).strict();
const issueEnvelope = z.object({ issues: z.array(issue).max(200), ...envelopeMetadata }).strict();
function metadataFrom(value: { verifiedAt?: string | undefined; source?: "snapshot" | "live" | undefined; refreshing?: boolean | undefined }) { return { ...(value.verifiedAt === undefined ? {} : { verifiedAt: value.verifiedAt }), ...(value.source === undefined ? {} : { source: value.source }), ...(value.refreshing === undefined ? {} : { refreshing: value.refreshing }) }; }
