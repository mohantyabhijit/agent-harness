...
export function CampaignPage({ api, campaignId, createIdempotencyKey = defaultIdempotencyKey, trueForgeBaseUrl }: CampaignPageProps) {
  ...
  const [approvedDigest, setApprovedDigest] = useState<{ readonly generation: number; readonly digest: string }>();
  const [campaignGeneration, setCampaignGeneration] = useState(0);
  const [previousCampaignId, setPreviousCampaignId] = useState(campaignId);
  ...
  if (previousCampaignId !== campaignId) {
    setPreviousCampaignId(campaignId);
    setCampaign(undefined);
    setError(undefined);
    setApprovalError(undefined);
    setSubmitting(false);
    setApprovedDigest(undefined);
    setCampaignGeneration(0);
  }
  ...
  const loadCampaign = useCallback(() => {
    ...
    void api.getCampaign(campaignId, controller.signal).then((loaded) => {
      if (!controller.signal.aborted && mountedGeneration.current === generation) {
        ...
        setSubmitting(false);
        setApprovedDigest(undefined);
        setCampaignGeneration(generation);
        setCampaign(loaded);
      }
  ...
  useEffect(() => {
    ...
    const expiry = campaign.approvals
      .filter(({ status, expiresAt }) => status === "approved" && expiresAt !== undefined)
      .map(({ expiresAt }) => Date.parse(expiresAt ?? ""))
      .filter((instant) => Number.isFinite(instant) && instant > Date.now())
      .toSorted((left, right) => left - right)[0];
    if (expiry === undefined) return;
    const timer = globalThis.setTimeout(loadCampaign, Math.min(2_147_483_647, Math.max(0, expiry - Date.now() + 1)));
  ...
  const approve = (confirmation: ApprovalConfirmation) => {
    ...
    void api.issueApproval(campaign.id, confirmation, key, controller.signal).then((approval) => {
      if (!controller.signal.aborted && mountedGeneration.current === generation) {
        setApprovedDigest({ generation, digest: approval.actionDigest });
        setSubmitting(false);
        approvalLocked.current = false;
        loadCampaign();
      }
  ...
  const alreadyApproved = proposal !== null && (approvedDigest?.generation === campaignGeneration && approvedDigest.digest === proposal.actionDigest || campaign.approvals.some((approval) => approval.actionDigest === proposal.actionDigest && approvalIsActiveOrConsumed(approval)));
