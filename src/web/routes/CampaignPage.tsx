...
type PublicationState = Readonly<{ routeIdentity: object; proposalId: string; status: "submitting" | "unknown" | "error"; result?: PublicationResult }>;
...
export function CampaignPage({ api, campaignId, createIdempotencyKey = defaultIdempotencyKey, createFinalizationKey = defaultFinalizationKey, trueForgeBaseUrl }: CampaignPageProps) {
  ...
  const publishApprovedAction = () => {
    ...
    void api.publishApprovedAction(campaign.id, approval.id, proposal.action, controller.signal).then((result) => {
      ...
      setPublication({ routeIdentity, proposalId: proposal.proposalId, status: reason instanceof OpenQuestApiError && reason.code === "publication_outcome_unknown" ? "unknown" : "error" });
    });
  };
  ...
  const currentPublication = publication?.routeIdentity === routeIdentity && publication.proposalId === proposal?.proposalId ? publication : undefined;
