...
import { OpenQuestApiError, type ApprovalConfirmation, type CampaignSnapshot, type OpenQuestApi, type PublicationResult } from "../api.js";
...
import { FixExplanationPanel } from "../components/FixExplanationPanel.js";
import { OpenQuestAgentThread } from "../components/OpenQuestAgentThread.js";
import { QualityGate } from "../components/QualityGate.js";
import { PublicationAction } from "../components/PublicationAction.js";

interface CampaignPageProps {
  readonly api: Pick<OpenQuestApi, "getCampaign" | "issueApproval"> & Partial<Pick<OpenQuestApi, "finalizeCampaign" | "runCampaignAction" | "publishApprovedAction">>;
...
type RefreshReason = Readonly<{ kind: "initial" }> | Readonly<{ kind: "post" }> | Readonly<{ kind: "action" }> | Readonly<{ kind: "publication" }> | Readonly<{ kind: "expiry"; key: string }>;
...
type PublicationState = Readonly<{ routeIdentity: object; proposalId: string; status: "submitting" | "unknown" | "reconciled" | "error"; result?: PublicationResult; message?: string }>;

const REFRESH_DEADLINE_MS = 10_000;

export function CampaignPage({ api, campaignId, createIdempotencyKey = defaultIdempotencyKey, createFinalizationKey = defaultFinalizationKey, operatorCapability, trueForgeBaseUrl }: CampaignPageProps) {
  ...
  const loadCampaign = useCallback((reason: RefreshReason = { kind: "initial" }, authorityExpiresAt?: string) => {
    ...
    void api.getCampaign(campaignId, controller.signal).then((loaded) => {
      ...
      if (!controller.signal.aborted && routeEpoch.current === epoch && readSequence.current === sequence) {
        ...
        setApprovedProposal(undefined);
        setError(undefined);
        if (reason.kind === "publication") {
          setPublication((current) => current?.routeIdentity === routeIdentity && current.status === "unknown" && publicationWasReconciled(loaded) ? { ...current, status: "reconciled" } : current);
        }
        setCampaign(loaded);
  ...
  const publishApprovedAction = () => {
    if (campaign === undefined || campaign.approvalProposal === null || api.publishApprovedAction === undefined) return;
    const proposal = campaign.approvalProposal;
    if (proposal.action.action !== "push_branch" && proposal.action.action !== "create_pr") return;
    const approval = activeApprovalForProposal(campaign);
    if (approval === undefined) return;
    const controller = new AbortController();
    const epoch = routeEpoch.current;
    setPublication({ routeIdentity, proposalId: proposal.proposalId, status: "submitting" });
    void api.publishApprovedAction(campaign.id, approval.id, proposal.action, controller.signal).then((result) => {
      if (controller.signal.aborted || routeEpoch.current !== epoch) return;
      setPublication({ routeIdentity, proposalId: proposal.proposalId, status: "submitting", result });
      ...
      setPublication({ routeIdentity, proposalId: proposal.proposalId, status: outcomeUnknown ? "unknown" : "error", ...(outcomeUnknown || !(reason instanceof Error) ? {} : { message: reason.message }) });
  ...
  const currentPublication = publication?.routeIdentity === routeIdentity && (proposal === null || publication.proposalId === proposal.proposalId) ? publication : undefined;
  const activeApproval = activeApprovalForProposal(campaign);
  ...
  return <main className="campaign-shell">
    ...
    <div className="campaign-layout"><div className="campaign-main">{campaign.issueBrief === null ? <section className="campaign-panel" role="alert"><h2>Issue analysis unavailable</h2><p>OpenQuest did not return a valid source-backed brief. Finalization and sandbox work remain locked.</p></section> : <IssueBriefPanel brief={campaign.issueBrief} finalized={campaign.status !== "policy_review"} onFinalize={finalizeBrief} submitting={currentFinalization === "submitting"} />}{campaign.fixExplanation === null ? null : <FixExplanationPanel explanation={campaign.fixExplanation} />}{currentFinalization === "error" ? <p className="campaign-error" role="alert">The issue brief could not be finalized. Reload the latest campaign facts and try again.</p> : null}<CampaignActions action={campaign.nextAllowedAction} onRun={runAction} {...(currentRunningAction === undefined ? {} : { running: currentRunningAction })} status={campaign.status} />{actionError?.routeIdentity !== routeIdentity ? null : <p className="campaign-error" role="alert">{actionError.message}</p>}{currentPublication?.status === "reconciled" ? <p className="publication-result" role="status"><strong>Publication reconciled</strong><br />The server confirmed that the pull request was opened after the transport response was lost.</p> : null}{proposal !== null && (proposal.action.action === "push_branch" || proposal.action.action === "create_pr") ? <PublicationAction action={proposal.action} {...(activeApproval === undefined ? {} : { approvalId: activeApproval.id })} onPublish={publishApprovedAction} {...(currentPublication?.status === "submitting" ? { submitting: currentPublication.result === undefined } : {})} {...(currentPublication?.result === undefined ? {} : { result: currentPublication.result })} {...(currentPublication?.message === undefined ? {} : { errorMessage: currentPublication.message })} {...(currentPublication?.status === "unknown" ? { error: "unknown" as const } : currentPublication?.status === "error" ? { error: "generic" as const } : {})} /> : null}<CampaignTimeline approvals={campaign.approvals} events={campaign.events} /><EvidencePanel evidence={campaign.evidence} references={campaign.externalReferences} /><QualityGate escalationReason={campaign.qualityEscalationReason} findings={campaign.qodoFindings} iteration={campaign.qodoIteration} status={campaign.status} /></div><aside className="campaign-side" aria-label="Agent and approval controls"><OpenQuestAgentThread operatorCapability={operatorCapability} sessionId={campaign.parentSessionId} {...(trueForgeBaseUrl === undefined ? {} : { trueForgeBaseUrl })} />{currentRefresh?.status === "pending" ? <p aria-label="Refreshing campaign facts" className="campaign-error" role="status">Refreshing authoritative campaign facts…</p> : currentRefresh?.status === "failure" ? <div aria-label="Campaign facts refresh failed" className="campaign-error" role="alert"><p>Campaign facts could not be refreshed. The loaded campaign remains visible, but campaign actions remain locked until the refresh succeeds.</p><button onClick={() => { refreshCampaign(currentRefresh.reason); }} type="button">Retry campaign refresh</button></div> : null}{proposal === null ? <section className="campaign-panel pending-proposal" aria-labelledby="proposal-pending-heading"><p className="eyebrow">Human approval boundary</p><h2 id="proposal-pending-heading">Exact proposal is pending</h2><p>The server has not published a current, validated action payload. Approval remains unavailable until a durable proposal matches this campaign and commit.</p><button disabled type="button">Approval unavailable</button></section> : <ChangeBrief approved={alreadyApproved} onApprove={approve} proposal={proposal} submitting={submittingRoute === routeIdentity || currentRunningAction !== undefined} />}{approvalError?.routeIdentity !== routeIdentity ? null : <p className="campaign-error" role="alert">{approvalError.message}</p>}</aside></div>
  </main>;
}
...
function activeApprovalForProposal(snapshot: CampaignSnapshot): CampaignSnapshot["approvals"][number] | undefined {
  const proposal = snapshot.approvalProposal;
  if (proposal === null || (proposal.action.action !== "push_branch" && proposal.action.action !== "create_pr")) return undefined;
  return snapshot.approvals.find((approval) => approval.status === "approved" && isCurrentlyActive(approval) && approval.action === proposal.action.action && approval.proposalId === proposal.proposalId && approval.actionDigest === proposal.actionDigest && approval.expectedCampaignVersion === proposal.expectedCampaignVersion);
}

function publicationWasReconciled(snapshot: CampaignSnapshot): boolean {
  return snapshot.status === "pull_request_open" && snapshot.externalReferences.some(({ kind, value }) => kind === "pull_request" && /^https:\/\/github\.com\/[^/]+\/[^/]+\/pull\/[1-9][0-9]*$/u.test(value));
}
