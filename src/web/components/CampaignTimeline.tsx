...
export function CampaignTimeline({ events, approvals }: CampaignTimelineProps) {
  const entries = [
    ...events.map((event) => ({ id: `event-${event.id}`, occurredAt: event.occurredAt, label: eventLabels[event.eventType] ?? humanize(event.eventType) })),
    ...approvals.map((approval) => ({ id: `approval-${approval.id}`, occurredAt: approval.issuedAt, label: `${approvalActionName(approval.action)} approval ${approval.status}` })),
  ...
  return <section aria-labelledby="timeline-heading" className="campaign-panel">
    ...
    {entries.length === 0 ? <p>No durable campaign events have been recorded yet.</p> : <ol className="campaign-timeline">
      {entries.map((entry) => <li key={entry.id}>
        ...
        <div><strong>{entry.label}</strong><time dateTime={entry.occurredAt}>{formatTimestamp(entry.occurredAt)}</time></div>
      </li>)}
    </ol>}
  </section>;
}

function humanize(value: string): string { return value.split("_").map((word) => `${word.slice(0, 1).toUpperCase()}${word.slice(1)}`).join(" "); }
