...
export function App({ api, operatorCapability }: AppProps) {
  ...
  if (path === "/discover" && selectedSpaces.length > 0) return <><button className="disconnect" onClick={() => { setEnteredCapability(""); setConnected(false); }} type="button">Disconnect</button><DiscoverPage api={client} navigate={navigate} spaces={selectedSpaces} /></>;
