...
This repository includes `.pr_agent.toml`, which asks the Qodo GitHub App to run `/review` when a pull request is opened, reopened, or marked ready for review, and again when new commits are pushed to an open pull request. The configuration also requires test, security, and ticket-analysis review sections. Installing and authorizing the Qodo GitHub App remains an external repository-owner action; the file alone cannot grant Qodo access.
